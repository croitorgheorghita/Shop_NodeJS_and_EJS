...
exports.register= async (req,res)=>{
    ...
    if(!errors.isEmpty()){
     ...
     if(user){
         ...
         console.log(err)
         res.render('register', {errorMessage: err[0],isLogged: req.session.isLogged})
     }
     else{ 
...
exports.addProduct= async(req,res)=>{
    ...
    if(!errors.isEmpty()){
        console.log(errors.array()[0].msg)
        res.render('addProduct', {errorMessage: errors.array()[0].msg,isLogged: req.session.isLogged})
    }
    if(!image){
        //console.log(errors.array()[0].msg)
        err.push("The file must be a image")
        res.render('addProduct', {errorMessage: err[0],isLogged: req.session.isLogged})
    }
    else{ 
    imageUrl=image.path
    var product={
...
exports.addToCart= async (req,res)=>{
    ...
    if(index>=0){
       ...
       updateCart[index].quantity=Quantity
        ...
        updateCart.push({productId: mongoose.Types.ObjectId(req.body.id), quantity: 1})
    }
...
exports.takeOrder= async (req,res)=>{
    ...
    user.cart.forEach(element => {
        ...
        pdfkit.fontSize(17).text("Produs: "+ element.productId.title + '------ Pret $' +  element.productId.price + ' x ' + element.quantity+" = " + pret + '$')
        pretTotal=pretTotal+pret
    });
    pdfkit.fontSize(20).text("-----------------------------------------------------------")
